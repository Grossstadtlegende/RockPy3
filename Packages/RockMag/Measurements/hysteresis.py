...
class Hysteresis(measurement.Measurement):
    ...
    **Fitting**
    ...
    def format_vftb(self):
        ...
        raw_data['mag'] = raw_data['mag'].v * self.ftype_data.mass
        raw_data['std_dev'] = raw_data['std_dev'].v * self.ftype_data.mass
    ...
    def plt_hys(self, noshow=False):
        ...
        #     plt.show()
        fig = RockPy.Figure()
        corrected = fig.add_visual(visual='hysteresis', plt_input=self, name='corrected')
        if self.correction:
            self_copy = deepcopy(self)
            self_copy.reset_data()
            original = fig.add_visual(visual='hysteresis', plt_input=self_copy, name='original')
            comparison = fig.add_visual(visual='hysteresis', plt_input=[self, self_copy], name='comparison')
...
def plot_app2sat(m, sat_perc=80):
    dfp, dfm, ufp, ufm = m.get_df_uf_plus_minus(saturation_percent=sat_perc, ommit_last_n=0)

    x = np.linspace(0, m.max_field)
    m.calculate_ms_APP2SAT(saturation_percent=sat_perc)
    y = [m.approach2sat_func(i, m.results['ms'].v, m.results['hf_sus'].v, m.results['alpha'].v) for i in x]
    y2 = [m.approach2sat_func(i, m.results['ms'].v, m.results['hf_sus'].v, 0) for i in x]
    plt.plot(np.fabs(m.data['down_field']['field'].v), np.fabs(m.data['down_field']['mag'].v))
    plt.plot(np.fabs(m.data['up_field']['field'].v), np.fabs(m.data['up_field']['mag'].v))
    # plt.plot(x, y, 'r')
    plt.plot(x, y2, 'r')
    # plt.xlim([1, 2])
...
if __name__ == '__main__':
    ...
    vsm_file = RockPy.join(RockPy.test_data_path, 'vsm', 'LTPY_527,1a_HYS_VSM#[]_[]_[]#TEMP_300_K#STD000#.000')
    ...
    m = s.add_measurement(mtype='hys', fpath=vftb_file, ftype='vftb')
    # m.plt_hys()
